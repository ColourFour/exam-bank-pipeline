...
The Asterion handoff is now course-aware for the static 9709 study site. Supported course IDs are `p1` (Pure Mathematics 1), `p3` (Pure Mathematics 3), `m1` (Mechanics 1, sourced from paper family `p4`), and `s1` (Probability & Statistics 1, sourced from paper family `p5`). P3 remains the most developed runtime path. P1, M1, and S1 are scaffolded until official topic maps and reviewed student-runtime records are intentionally added.
