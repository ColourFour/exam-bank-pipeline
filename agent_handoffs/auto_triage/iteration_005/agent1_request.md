# Agent 1 Request - iteration_005

Plan one narrow extraction-quality improvement pass.

Selected target:
- Issue: `question_mark_total_mismatch`
- Current count: `28`
- Current hard failures: `71`
- Stop threshold: `50`

Top issue counts:
- `question_mark_total_mismatch`: 28
- `paper_total_mismatch`: 24
...
- Hard failures decrease, or `question_mark_total_mismatch` decreases by at least one.
...
```bash
.venv/bin/python -m exam_bank.cli triage-sample --input output_ocr_candidate/json/question_bank.json --output-root output_ocr_candidate/triage --iteration iteration_005 --issue-set hard-failures --target question_mark_total_mismatch --sample-size 30 --seed 1
```
- `triage_serve`:

```bash
.venv/bin/python -m exam_bank.cli triage-serve --iteration output_ocr_candidate/triage/iteration_005
```
- `full_ocr_rerun`:
...
```bash
.venv/bin/python -m exam_bank.cli auto-triage-compare --iteration agent_handoffs/auto_triage/iteration_005 --baseline-triage output_ocr_candidate/triage/iteration_005 --current output_ocr_candidate/json/question_bank.json --output output_ocr_candidate/triage/iteration_005/comparison.auto-iteration-005.json --test-status pass
```
