...
Current deterministic difficulty work is represented by `output/json/question_bank.difficulty_index.v1.json`, built by `exam-bank topic difficulty-index`. That sidecar may use mark events, topic routing, and advisory examiner-report/grade-threshold evidence, but it remains advisory and does not enable student-facing sequencing in v1. See [Difficulty Index Contract](DIFFICULTY_INDEX_CONTRACT.md).
